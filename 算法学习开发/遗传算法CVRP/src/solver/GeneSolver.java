...
import static helper.IndividualHelper.OXCrossForBestChild;
...
public class GeneSolver {
    ...
    public GeneSolver(List<Point> points, int population, int maxAge, double r, double v, int capacity) {
        this.individuals = Helper.createFirstGen(points, population, capacity);
    ...
    public List<Point> run() {
        ...
        while (age < maxAge) {
            ...
            for (int i = 0; i < population - nextGen.size(); i++) {
                ...
                Individual child = OXCrossForBestChild(father, mother);
                children.add(child);
            }
            ...
            System.out.println(individuals.get(0).fitness() + ", "
                + individuals.get(0).calculateDistance() + ", "

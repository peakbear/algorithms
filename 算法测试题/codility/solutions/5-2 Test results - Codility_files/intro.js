  ...
  */
  ...
  function _setHelperLayerPosition(helperLayer) {
    if (helperLayer) {
      ...
      // force helper layer transparency for elements that have offsetParents positioned as fixed
      var offsetParentPosition = _getPropValue(currentElement.element.offsetParent, 'position');
      if (offsetParentPosition === 'fixed') {
        helperLayer.classList.add('transparent');
      } else {
        helperLayer.classList.remove('transparent');
      }
    }
  }
  ...
  function _showElement(targetElement) {
    ...
    // lifted the magic number 80 'to get the text right' from _elementInViewport()
    var optionalBottomOffset = this._introItems[this._currentStep].position == 'top' ? 0 : 80;

    if (!_elementInViewport(targetElement.element, optionalBottomOffset) && this._options.scrollToElement === true) {
   ...
   * @param {Object} el
   * @param {Number} optionalBottomOffset
   */
  function _elementInViewport(el, optionalBottomOffset) {
    var rect = el.getBoundingClientRect();

    return (
      rect.top >= 0 &&
      rect.left >= 0 &&
      (rect.bottom+optionalBottomOffset) <= window.innerHeight &&
